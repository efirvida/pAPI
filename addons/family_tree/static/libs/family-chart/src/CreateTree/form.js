...
export function createForm({ datum, store, fields, postSubmit, addRelative, deletePerson, onCancel, editFirst }) {
  ...
  function submitFormChanges(e) {
    ...
    const fileInputs = Array.from(e.target.querySelectorAll('input[type="file"]'));

    const fileReadPromises = fileInputs.map(input => {
      const file = input.files[0]; // solo uno por input
    ...
    Promise.all(fileReadPromises).then(fileEntries => {
      formData.forEach((value, key) => {
        if (!datum.data) datum.data = {};
        if (!fileEntries.some(([fileKey]) => fileKey === key)) {
